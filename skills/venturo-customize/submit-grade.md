...
5. **POST** via one Bash `curl` (never print the key value):
    ```bash
...
6. **Report by status** (as returned, don't blindly retry):
   ...
   - `401` → "VIBESCORE_API_KEY tidak valid/absen — skor TIDAK terkirim."
    - `400` → if the `error` mentions an unknown/unexpected field (e.g. "unknown field", "unexpected field"), strip `session_name`, `compacted`, and `transcript_meta` from the body, resubmit once, and tell the user the backend doesn't yet support the new fields. Otherwise show the `error` field from the response body (payload rejected by contract).
